...
ggplot(data = penguins, 
  ...
  geom_point() + 
  scale_color_gradient(low = "cyan3", high = "magenta4") +
  theme_bw() + 
  labs(x = "Bill Length (mm)", 
       y = "Bill Depth (mm)", 
       color = "Body Mass (grams)")
```
...
```{r}
#| warning: false
# Method 1: Use Default geom_bar() stat
ggplot(data = penguins, 
...
```{r}
#| warning: false
# Method 2: Define geom_bar() stat = "identity" 
penguin_summary <- penguins |>
  count(species)

ggplot(data = penguin_summary, 
...
ggplot(penguins, aes(x = species, fill = island)) + 
  ...
  scale_fill_manual(values = c("darkorange", "darkblue", "turquoise3")) + 
  theme_bw()
```
...
#| warning: false
ggplot(penguins, aes(x = species, fill = island)) + 
  geom_bar(position = position_dodge2(preserve = "single")) + # makes all bars the same width
  scale_fill_manual(values = c("darkorange", "darkblue", "turquoise3")) + 
  theme_bw() + 
  labs(x = "Species", 
       y = "Count",
...
# theme_light() removes grey background
```
